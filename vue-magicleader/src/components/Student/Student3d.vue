<template lang="html">
    <div id='id-student3d'>
        ...
        <div class="waterfall-container" v-if="student3dList != ''">
...
export default {
    ...
    methods: {
        getStudent3dList() {
            ...
            this.log('this.student3dList', this.student3dList)
        },
        infiniteHandler( $state) {
            setTimeout(() => {
                ...
                if (img.length != 0) {
                    ...
                    $state.complete()
                    this.log('完成', this.student3dList.length);
                }
            }, 1000);
        },
    },
    updated() {
        if (this.$store.state.student2dList.length != undefined) {
                this.log('3d作品进度', this.student3dList.length, '/' ,this.$store.state.student3dList.length);
        }
    },
    components: {
        Waterfall,
